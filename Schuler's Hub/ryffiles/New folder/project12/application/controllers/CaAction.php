 ...
 extends CI_Controller {
	...
	public function __construct()
        ...
        public function ca_takeaction()
        {
			...
			$data['numrowscaaction'] = $this->Read_model->catakeactioncount();
			$data['numrowscatask'] = $this->Read_model->cataskcount();
			$data['numrowscatakeaction'] = $this->Read_model->catakeactioncount();
			#$data['numrowscountryambassador'] = $this->Read_model->countryambasadorcount();
    ...
    public function ambassodor_actions()
	{
		...
		$data['numrowscaaction'] = $this->Read_model->catakeactioncount();
		$data['numpointsca'] = $this->Read_model->countryambasadorpointscount($campusambasador);
		#$data['numrowscatakeaction'] = $this->Read_model->catakeactioncount();
	...
	public function actionsubmissions()
	{
		...
		$this->db->select('*');
		$this->db->from('action_reports');
		#$this->db->where('flag');#,0);
		#$this->db->join('campusambassador', 'reports.user_id = campusambassador.id'); 
		#$this->db->join('ca_task','reports.task_id = ca_task.id');
		$query = $this->db->get();
		$task_submissions =  $query->result(); 
		$data['tasksubmissions'] = $task_submissions;
		$data['campusambasadorscountryarray'] = $this->Read_model->countries();
		$data['campusambasadorsstatesarray'] = $this->Read_model->states_for_filter();
		$this->load->view('admin/include/header');
		$this->load->view('admin/action-submission',$data);
		$this->load->view('admin/include/footer');
	}

	public function ca_action_request(){

		$this->load->view('admin/include/header');
		$this->load->view('admin/action-request');
		$this->load->view('admin/include/footer');
	}
