...
class Admin_model extends CI_Model {
    ...
    public function add_action($title,$description,$image_url,$file_url,$points)
    {
      $query = "INSERT INTO `the_action`(`title`, `description`,`image`, `file`, `point`) VALUES ('{$title}','{$description}','{$image_url}','{$file_url}','{$points}')";
      if($this->db->query($query))
      {
          return 1;
      }
    ...
    public function add_actionreport($task,$file_url,$user_id)
    {
      $query = "INSERT INTO `the_actionreport`(`task_id`, `file_url`, `user_id`) VALUES ('{$task}','{$file_url}','{$user_id}')";
      if($this->db->query($query))
      {
          return 1;
      }
